...
class Command(pdu.PDU):
    ...
    def generate_params(self):
        ...
        #print("self.params_order:",self.params_order) ### DEBUG
        for field in self.params_order:
            ...
            #print(param, type(param.type)) ### DEBUG
            if self.field_is_optional(field):
                ...
                if param.type is int:
                    ...
                    if value:
                        body += value
        #print('body:',body, type(body)) ### DEBUG
    ...
    def _parse_string(self, field, data, pos):
        ...
        #print('field, data, pos:',field, type(field), data,type(data), pos, type(pos)) ### DEBUG
        null_ch = chr(0)
        if type(data) is bytes: null_ch = b'\x00'
        end = data.find(null_ch, pos)
        length = end - pos
