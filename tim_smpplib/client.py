...
class Client(object):
    ...
    def _bind(self, command_name, **kwargs):
        ...
        try:
            ...
            raise exceptions.ConnectionError('Bind socket timeout')
        if resp.is_error():
    ...
    def unbind(self):
        ...
        try:
            ...
            raise exceptions.ConnectionError('Unbind socket timeout')

    def send_pdu(self, p):
        """Send PDU to the SMSC"""

        #print('self.state:', self.state) ### DEBUG
        #print('p.command:', p.command) ### DEBUG
        ...
        logger.info('Sending %s PDU', p.command)

        generated = p.generate()
        #print('generated:', generated, type(generated)) ### DEBUG
        #logger.debug('>>%s (%d bytes)', binascii.b2a_hex(generated.encode('utf-8')),
        logger.debug('>>%s (%d bytes)', binascii.b2a_hex(generated),
            len(generated))

        sent = 0

        #generated_b = generated.encode('utf-8')
        #print('generated_b:',generated_b, generated) ### DEBUG
        while sent < len(generated):
            ...
            try:
                ...
                logger.warning(e)
                raise exceptions.ConnectionError('Send PDU socket error')
            if sent_last == 0:
                raise exceptions.ConnectionError('Send PDU sent_last == 0')
            sent += sent_last
    ...
    def read_pdu(self):
        ...
        try:
            ...
            logger.error('Read PDU socket timeout')
            raise
        except socket.error as e:
            logger.warning(e)
            raise exceptions.ConnectionError('Read PDU socket error')
        if not raw_len:
            raise exceptions.ConnectionError('Read PDU no raw_len')
        ...
        #print('p:',p.__dict__) ### DEBUG
        logger.info('Read %s PDU', p.command)
        ...
        if p.is_error():
            ...
            #print('SET STATE: Command %s set %s state' % (p.command, self.state)) ### DEBUG
        ...
        while True:
            try:
                ...
                if p.command == 'unbind':  # unbind_res
                    ...
                    logger.info('SubmitSM command received')
                    self.message_sent_handler(pdu=p)
                elif p.command == 'deliver_sm':
                    logger.info('DeliverSM command received')
